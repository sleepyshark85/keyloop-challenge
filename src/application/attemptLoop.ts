 ...
 *
 * ── `booking_conflicts_total` HAS EXACTLY ONE INCREMENT SITE (design decision 2) ────────────────
 *
 * Two calls below, both to the same `Counter`: `refuse()` on `exhausted`/`capped`, and the
 * success return when the winning attempt was preceded by a conflict (`absorbed` — the request
 * met contention and survived it). `outcome` is genuinely a parameter here, not a literal
 * repeated at three call sites, which is `tests/architecture/ambiguity-containment.test.ts`'s own
 * `conflict-counter-increment` marker's point: the concept is the label carried through, not one
 * spelling of it.
 *
...
import { bookingConflictsTotal, tracer } from '../platform/telemetry.js';
...
export async function runAttemptLoop<TRow, TAbort>(
  ...
  const refuse = (
    ...
    bookingConflictsTotal.add(1, { resource, outcome: exit === 'exhausted' ? 'refused' : 'capped' });
  ...
  for (let attempts = 1; attempts <= structuralBound; attempts += 1) {
    ...
    if (result.tag === 'row') {
      ...
      if (lastConflictResource !== null) {
        bookingConflictsTotal.add(1, { resource: lastConflictResource, outcome: 'absorbed' });
      }
    ...
    switch (outcome.kind) {
      ...
      case 'bad-reference':
        return { kind: 'aborted', value: await onBadReference(outcome.constraint, currentBayId, currentTechnicianId) };
      ...
      case 'other':
        // Not this loop's to interpret — `classify` is total, and everything it cannot name is a
  ...
  // conflict empties a list by attempt `structuralBound`, and every classification either returns
  // or prunes.
