...
booked interval exhausts the cap before the candidates.
...
is faithful to it — the **magnitude** is new. One-in-a-thousand and one-in-six are different decisions
in the same words, and §4.1 promises the first.

## Considered options

| Option | Argued |
|---|---|
| **Nothing — keep the residual** | Good: documented (§11 R-4), the code faithful to it. Bad: §4.1 promises the opposite inside the single source of truth; 17.5 % is not a residual chosen knowing the number |
| **Cap-1 — raise `BOOKING_ATTEMPT_CAP` to `|bays| + |technicians|` or above** | Good: it **works, and structurally** — Bound-2 guarantees a list empties by then, so the refusal is unreachable rather than unlikely; one config value, no code. Bad: the cap is a **latency** guard while Bound-2 already bounds termination, so at 40 a worst-case refusal costs 40 attempts of three round trips each, and refusal is the answer a client waits longest for. **Rejected on that cost alone; it stays available to the gate** |
| **Order-E — one advisory read, ordering free-first, membership unchanged. Chosen** | Good: it spends the cap on candidates that might succeed — the *cause* — and never changes membership, so no exit's reachability moves. Bad: concurrent requests now agree at binary granularity (Order-D's objection); replay needs the snapshot too |
| **Filter-1 — remove busy candidates from the list** (ADR-0033's refused option, kept as evidence) | Good: the obvious implementation, cheapest to write. Bad: a removing read can empty the list, which §6.2 routes to `500`/`422` and never `409` — a `500` for a merely full dealership, or a `409` minted from a read, which ADR-0016 forbids. §2.1's shape with the `if` in the query planner |
...
stays the only adjudicator: a wrong snapshot costs **attempts** and cannot itself refuse. That is not
a claim that refusal becomes unreachable — the cap can still spend sixteen attempts while capacity
exists, which is why `capped` becomes unlikely rather than impossible below.
...
utilisation ordering, not occupancy awareness — and Order-D's objection comes with it, weakened, as
the residual below.
...
- Ordering stays pure: `busy` arrives as a parameter, `domain-is-pure` letting nothing in; reversible
  for one argument.

**Bad, or deferred**

- **Two of ADR-0009's consequences become false.** *"the residual … now at depth 17 rather than
  count 17"*: it was at **occupancy** 17 too, with no concurrency. And *"work is not balanced across
  resources … knowingly given up"*: now partly taken. A third is weakened — a recorded seed no longer
  reproduces a run alone, the permutation depending on the snapshot too.
- **`capped` becomes unlikely, not impossible.** Attempt 16 now needs fifteen conflicts among
  candidates the snapshot called free: the signal is restored **probabilistically**, Cap-1 structurally.
- **The residual this record does not close.** Under a burst every racer reads the same snapshot, so
  ...
  argument. **QS-16 is its falsifier and it may fail.**
