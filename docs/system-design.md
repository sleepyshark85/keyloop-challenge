...
| **11** | [Risks and technical debt](arc42/11-risks-technical-debt.md) | The register is generated by `npm run docs:build` from two sources, because debt arrives in two shapes: **an ADR with `status: proposed`**, an open decision… |
