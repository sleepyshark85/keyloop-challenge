...
consequential fact about the context, justified in §3.1.2 rather than left unstated.
...
| **Technician** | **A resource, not a user.** Consumed by an appointment, exclusively, for its whole duration (A-2) | Nothing crosses. Recorded because modelling a technician as a user is a strong temptation and a much larger scope |
...
client for the five operations and the contention demo, seeded by `npm run harness:seed`, which
prints the environment its scripts need — so the boundary is reachable from a terminal alone.
...
marked **†** are carried into §11 as debt, with what would trigger them.
...
- **Technician shifts, rotas, holidays and absence** — any per-technician working calendar
  ...
  exclusion constraint enforces and need its own mechanism and quality scenario. Dealership opening
  hours are **not** excluded: they are validated, being decidable from the request alone.
...
- **Authentication, authorisation, sessions, rate limiting and per-actor audit**
  ([ADR-0034](../adr/0034-the-caller-is-a-user-and-the-system-does-not-name-the-role.md), GC-2). **†** The service is therefore unsafe on any reachable network, acceptable
  only because §7 deploys a single local container. Vehicle ownership is checked, but as validation —
  no appointment records who booked, moved or cancelled it.
...
- Pagination, filtering and sorting beyond the availability query's own.
- Notifications, reminders or confirmations by email or SMS.
...
- Waitlists, overbooking, priority or emergency jobs, and any queue-jumping policy. Each is a
  scheduling *policy* question — where a scheduler earns its keep, and exactly why none of it can be
  done convincingly inside an assessment. **†**
