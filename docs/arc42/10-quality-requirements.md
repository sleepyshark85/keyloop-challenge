...
| **QS-13** | 4 | **The window is visible and conflicts are counted.** With an in-memory exporter, a booking that retries once then succeeds gives an `availability.candidates` span **ending before** the first `appointment.insert` begins, exactly two `appointment.insert` spans, the failed one carrying `db.sqlstate=23P01` and `db.constraint`, and `booking_conflicts_total{resource="bay",outcome="absorbed"} == 1` with no `refused` increment | `tests/integration/telemetry-booking.test.ts` |
