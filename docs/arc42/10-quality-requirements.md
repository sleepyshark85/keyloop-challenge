...
| **QS-9** | 1 | **Opening hours across a DST transition (ADR-0001).** For a dealership in `Europe/London` open 09:00–17:00 local, generated instants on the days surrounding both the March and October transitions are accepted if and only if their **local** rendering lies within the window — in particular the instant that is 08:30 local but 09:30 UTC is rejected, and its counterpart accepted, on both sides of each transition. Also asserts that a 60-minute job starting 00:30 local on a spring-forward night ends 02:30 local (§8.3) | `tests/property/opening-hours-dst.test.ts` |
