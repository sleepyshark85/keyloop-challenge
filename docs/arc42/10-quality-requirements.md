...
What *"under any interleaving"* means operationally, since three scenarios use the phrase: the
...
|---|---|---|---|
| **QS-1** | 1 | **No bay overlap.** Given a dealership with one free bay over `[09:00, 10:00)`, when *N* = 20 booking requests for that interval are released simultaneously, then **exactly one** `appointment` row exists with `status <> 'cancelled'` for that bay over any overlapping range, the other 19 receive `409` with `type=/problems/no-capacity`, and the violated constraint reported by PostgreSQL is named `no_bay_overlap`. *The constraint name is observed on stdout — one structured `booking.conflict` line per `23P01` carrying the constraint, the resource minted from it and the attempt number — because it is in neither the response body nor the table, and an outside-in test may read only those three. ADR-0018's locks are what make the name deterministic: every loser conflicts with a committed row (§6.1)* | `tests/concurrency/no-bay-overlap.test.ts` |
...
**What is deliberately not here.** There is no scenario for availability being *fresh*: it is advisory
by contract (§4.1, §6.5), and asserting freshness would assert the property the design gives up on
purpose. There is none for authentication, shift modelling or appointment history — §3.3 excludes them
and §11 carries them as debt. And QS-14 is the only performance scenario, which is what "ranked last"
looks like.

`tests/architecture/` (QS-10, QS-12) and `tests/performance/` (QS-14) belong to the test-engineer under
`CLAUDE.md` §5, enforced by path in both directions by `.claude/hooks/guard-paths.mjs`.
