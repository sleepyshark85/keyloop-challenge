...
Each scenario is written so it can be executed, and names the test that enforces it — a link nothing
checks (§11 R-8).
...
| **QS-11** | 2 | **The error taxonomy is total and stable.** Every row of §8.6's status table is reachable and produces that status and that `type`, as `application/problem+json`. Specifically: an out-of-hours request is `400` and **not** `409`; an unknown vehicle is `422` and not `404`; a vehicle not owned by the named customer is `422` and not `403`; a contended booking is `409` with `resource` set; moving a cancelled appointment is `409` with a *different* `type` from a contended one. Also asserts the emitted OpenAPI document matches the committed one. *As built, seven of the eight rows are in scope and every one is reached end-to-end, the `500` included, through seeded reference data rather than a stub; `/problems/appointment-not-confirmed` needs rescheduling and joins at slice 06, and the OpenAPI half is slice 09's and unasserted. The sweep runs ∀rows ∃input, so it cannot fail on a response outside the table — and two are, measured (§8.6). ADR-0024 turns the residual into the falsifiable ∀responses ∃row over a hostile corpus, at slice 06* | `tests/contract/error-taxonomy.test.ts` |
