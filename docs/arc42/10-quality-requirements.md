...
| **QS-9** | 1 | **Opening hours across a DST transition (ADR-0001).** For a dealership in `Europe/London` open 09:00–17:00 local, generated instants around both the March and October transitions are accepted if and only if their **local** rendering lies within the window — in particular `2026-03-28T08:30:00Z` renders 08:30 local (GMT) and is **rejected**, while its counterpart `2026-03-29T08:30:00Z` — the same UTC wall time on the far side of the spring-forward transition — renders 09:30 local (BST) and is **accepted**. *(Corrected 2026-09-04 under the human's O-13 ruling, which is propagated here rather than re-decided: as first written this clause read "the instant that is 08:30 local but 09:30 UTC", which describes `Europe/London` at UTC−1, an offset the zone never has. UTC and local were transposed. The substance of the scenario is unchanged; the pair above is measured, not reasoned about.)* Also asserts that a 60-minute job starting 00:30 local on a spring-forward night ends 02:30 local (§8.3). *As built at slice 01, what "generated instants" means is narrower than this scenario's phrasing and is recorded rather than left to the reader: a stratified `fc.oneof` of three strata — ±90 minutes around each of the two 2026 transition instants, ±7 days around each at any second of the day, and the whole of 2026 at minute granularity — crossed with durations of 1–480 minutes and a generated weekly schedule in which any day may be closed. **One zone and one year.** A second zone, a southern-hemisphere transition and a zone whose offset is not a whole number of hours are all untested; §11 carries that. The worked AC-2 pair and AC-3 are asserted as fixed examples beside the properties, not generated, because a measured pair is stronger evidence than a shrunk counterexample for the case the scenario names. Per ADR-0013 the test reaches the modules through the built artifact under `dist/`, never `src/`* | `tests/property/opening-hours-dst.test.ts` |
| **QS-10** | 2 | **The layering is the ruleset, and the ruleset runs.** `depcruise` over `src/` and `tests/` with `.dependency-cruiser.js` exits 0. Additionally, injecting a known violation of each of `domain-is-pure`, `sql-only-in-persistence`, `http-must-not-reach-persistence` and `outside-in-tests-do-not-import-src` into a temporary fixture tree produces an error naming that rule — so the suite proves the rules **fire**, not merely that they parse. A ruleset that has never rejected anything is not evidence (§1.2 goal 2). *As built at slice 01: `domain-is-pure` carries **two** plants, not one — an import out of the domain (`src/domain/bad.ts → src/platform/config.ts`) and the **intra-domain** form (`src/domain/interval.ts → src/domain/duration.ts`), because the rule as first written permitted the second by construction and the single plant could not have shown it. The suite also guards that the cruise happened: every planted file must appear in the analysed set, so a fixture the cruise never looked at fails loudly instead of passing quietly* | `tests/architecture/layering.test.ts` |
...
| **QS-12** | 3 | **Ambiguity containment.** The three changes §1.2 goal 3 names are each confined to one module: duration arithmetic appears only in `src/domain/duration.ts` (A-1); the occupancy interval only in `src/domain/interval.ts` (A-4); wall-clock and IANA-zone reasoning — any use of `Intl.DateTimeFormat` with a `timeZone`, or of a dealership's `time_zone` — only in `src/domain/openingHours.ts` (ADR-0001). Response measure: **one source file plus one migration**. Asserted by scanning the tree, so it fails the day someone inlines an hour of arithmetic in a route handler. *As built at slice 01, two qualifications the scenario should carry. First, **the response measure is assumed, not measured** — the scan asserts containment today; nothing has yet made one of these three changes and counted the files it touched, and nothing will until a slice does. Second, the corpus is `src/**/*.ts` and is still small, so a marker matching in exactly one file is a weaker claim now than it will be at slice 08; §11 records what the scan does not catch* | `tests/architecture/ambiguity-containment.test.ts` |
...
**Two directories in this table needed an ownership ruling, and it was made.** `tests/architecture/`
(QS-10, QS-12) and `tests/performance/` (QS-14) were not in `CLAUDE.md` §5's table. §8.5 stated the
architect's proposal and flagged it for Gate B rather than assuming it; **the human ruled both to the
test-engineer on 2026-09-04**, `CLAUDE.md` §5 now lists them, and `.claude/hooks/guard-paths.mjs`
enforces it by path in both directions. Recorded here as settled so this paragraph does not keep
